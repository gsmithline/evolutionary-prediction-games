...
Section('training', 'Hyperparameters').params(
    ...
    flip_prob=Param(float, 'Probability of random horizontal flip', default=0.5),
    output_file=Param(str, 'Output file path', required=True),
)
...
@param('training.flip_prob')
def make_dataloaders(train_dataset=None, val_dataset=None, batch_size=None, num_workers=None, flip_prob=None):
    paths = {
        'train': train_dataset,
        'test': val_dataset
...
def evaluate(model, loaders):
    ...
    with ch.no_grad():
        for name,flip in [
            ...
            print(f'Evaluating {name}, flip={flip}')
            ...
            print(f'{name}/flip={flip} accuracy: {acc * 100:.1f}%')
            results[(name,flip)] = acc
    return results
...
if __name__ == "__main__":
    ...
    results['config'] = config.content
    results['total_time'] = end_time - start_time
    print(results)
    with open(config['training.output_file'],'wb') as f:
